...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalthree/ \
                                     --checkfiles BubbleSort.java --multicomments 4 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalthree \
                                     --checkfiles BubbleSort.java --fragments "println(" --fragmentcounts 4
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k "println()" fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/test/java/practicalthree \
                                     --checkfiles TestBubbleSort.java --fragments "@Test" --fragmentcounts 12
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces exactly k lines of output
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --outputlines 4
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces the required fragment
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "F"
  determine_exit_code $?
  # --> GatorGrader CHECK: running the program produces the required fragment
  python3 gatorgrader/gatorgrader.py --nowelcome --commands "gradle -q --console plain run" --fragments "4"
  determine_exit_code $?
